...
# **Additional Notes**
#
# - On message of the day (MOTD) screen there will be also the Pi Model, the
#   version of RetroPie-Setup and the RetroArch version as well as the SDL
#   version displayed. Note the extra green lines in output.
...
# - To get locale specific measurements (metric/imperial) for temperatures, set the
#   environment variable `LC_MEASUREMENT` in your `~/.bashrc` like this: `export
#   LC_MEASUREMENT=en_US` for imperial (Fahrenheit). Default is
#   `LC_MEASUREMENT=en_GB`, thus metric (Celsius).
# - RetroPie-Setup may revert to the pristine bashwelcometweak in some conditions. If
#   this happens, re-execute this scriptmodule again.

rp_module_id="rb_bashwelcome"
...
function install_rb_bashwelcome() {
    remove_rb_bashwelcome
    ...
    for i in "${!logo[@]}"; do
        ...
        case "$k" in
            ...
            2)
                ...
                ;;
            3)
                out+="${fggrn}RetroPie $(cat /opt/retropie/VERSION)"
...
function gui_rb_bashwelcome() {
    ...
    if [[ -n "$choice" ]]; then
        case "$choice" in
        1)
            rp_callModule rb_bashwelcome install
            printMsgs "dialog" "Installed $title."
            ;;
        2)
            rp_callModule rb_bashwelcome remove
